...
Requirements:
- Clean, well-structured, responsive UI
- Return ONLY the code in a single HTML file, inside a Markdown fenced code block
                  `.trim()
                }
              ]
            }
      ...
      toast.error(error?.message || "Failed to generate. Check console for details.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-6 lg:px-16">
        ...
        <div className="w-full py-6 rounded-xl bg-[#141319] mt-5 p-5">
          ...
          <p className='text-gray-400 mt-2 text-[16px]'>Sketch it in words. See it in code.</p>

          <p className='text-[15px] font-[700] mt-4 text-gray-400'>Framework</p>
          <Select
            className='mt-2'
            options={options}
            value={frameWork}
          ...
          <p className='text-[15px] font-[700] mt-5 text-gray-400'>Describe your component</p>
          <textarea
            onChange={(e) => setPrompt(e.target.value)}
            value={prompt}
            className='w-full min-h-[200px] rounded-xl bg-[#09090B] mt-3 p-3 text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-orange-500 resize-none'
            placeholder="Turn your UI idea into clean, ready-to-use code."
          ></textarea>
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <button
              onClick={getResponse}
              className="flex items-center p-3 rounded-lg border-0 bg-gradient-to-r from-orange-400 to-red-500 px-5 gap-2 transition-all hover:opacity-80 hover:scale-105 active:scale-95"
            >
        ...
        {/* Right Section */}
        <div className="relative mt-8 w-full h-[80vh] bg-[#141319] rounded-xl overflow-hidden">
          {
            !outputScreen ? (
              <div className="w-full h-full flex items-center flex-col justify-center">
                <div className="p-5 w-[70px] flex items-center justify-center text-[30px] h-[70px] rounded-full bg-gradient-to-r from-orange-400 to-red-500">
                  <HiOutlineCode />
                </div>
              ...
              <>
                ...
                <div className="bg-[#17171C] w-full h-[50px] flex items-center gap-3 px-3">
                  <button
                    onClick={() => setTab(1)}
                    className={`w-1/2 py-2 rounded-lg transition-all ${tab === 1 ? "bg-orange-500 text-white" : "bg-zinc-800 text-gray-300"}`}
                  >
                    Code
                  </button>
                  <button
                    onClick={() => setTab(2)}
                    className={`w-1/2 py-2 rounded-lg transition-all ${tab === 2 ? "bg-orange-500 text-white" : "bg-zinc-800 text-gray-300"}`}
                  >
                    Preview
                  </button>
                </div>
